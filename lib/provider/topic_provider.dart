    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['data'] != null && data['data']['data'] != null) {
          List<dynamic> topicsData = data['data']['data'];
          int doneCount =
              topicsData.where((topic) => topic['hasProgress'] == true).length;
          int total = data['data']['meta']['total'] ?? topicsData.length;
          _completed = doneCount;
          _total = total;
          _topics =
              topicsData.map((item) => TopicModel.fromJson(item)).toList();

          print('Fetched topics count: ${_topics.length}');
        } else {
          print('Invalid API response structure: ${response.body}');
        }
        _isLoading = false;
        notifyListeners();
      } else {
